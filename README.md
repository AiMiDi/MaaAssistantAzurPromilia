...
- 日常任务奖励：利用默认排序检查首屏，领取一次后重新识别；遇到已领取或未完成停止下翻，独立检查 20/40/60/80/100 活跃度宝箱。已实测领取每日登录奖励和 20 活跃度宝箱，无新增奖励的完整流程已通过；更高档宝箱仍待实际达标验证，界面标记为测试任务。
- 新增“一键领取 · 已适配奖励”：邮件、签到、日常、周常、小小侠客养成记、星之旅录、成就积分及普罗米利亚游记。旅录先检查底部阶段奖励再处理任务；190 积分档已实领；普罗米利亚游记14级奖励已实领，任务前后均检查等级奖励。周常及成就目前实测的是未达标跳过分支，完整范围见[奖励领取说明](docs/REWARDS.md)。
...
场景寻路的[预研方案](docs/NAVIGATION_RESEARCH.md)已整理：以 MaaEnd 核心技术配合蓝色星原地图、通行和控制数据，先验证家园起点到订单看板的路线闭环，后续扩展多设施和大世界。当前尚未实现角色自动寻路。
